	...
	dynamic public class TweenGroup extends Proxy implements IEventDispatcher {
		...
		public function TweenGroup($tweens:Array=null, $DefaultTweenClass:Class=null, $align:String="none", $stagger:Number=0) {
			...
			if (!_classInitted) {
				...
				try {
					_TweenMax = (getDefinitionByName("com.blenderbox.motion.TweenMax") as Class); //Checking "if (tween is _TweenMax)" is twice as fast as "if (tween.hasOwnProperty("combinedTimeScale"))". Storing a reference to the class this way prevents us from having to import the whole TweenMax class, thus saves a lot of Kb.
				} catch ($e:Error) {
					_TweenMax = Array;
				}
